...
def train_epoch(
    ...
    with tqdm(
        ...
        desc=f"Train model using {rank} GPU in {epoch} epoch config.device is = {config.device}",
    ) as pbar:
...
def train_model(rank, world_size, args):
    ...
    config.tconfig.device = torch.device(
    ...
    train_loader, train_sampler = build_data_loader(
        vocab, train_data, args, shuffle=True
    )
    ...
    optimizer_grouped_parameters = [
        {
            ...
            "weight_decay": config.tconfig.weight_decay,
        },
        {
            "params": [
                p
    ...
    optimizer = AdamW(
        optimizer_grouped_parameters,
        lr=config.tconfig.learning_rate,
        eps=config.tconfig.adam_epsilon,
    )
    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=config.tconfig.warmup_steps,
    ...
    for step in trange(config.tconfig.n_epoch, desc="Epoch"):
        ...
        if master and best_score < score:
            ...
            if isinstance(
                model, DistributedDataParallel
            ):  # isinstance(a:object,b:classtype) => 만약 a가 b의 subclass instance 이거나 그냥 instance 인경우는 True
                model.module.save(best_epoch, best_loss, best_score, args.save)
            else:
                model.save(best_epoch, best_loss, best_score, args.save)
            print(
                f">>>> rank: {rank} save model to {args.save}, epoch = {best_epocs}, loss = {best_loss:.3f}, score = {best_score:.3f}"
            )
