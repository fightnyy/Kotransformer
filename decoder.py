...
def get_attn_decoder_mask(seq):
    ...
    subsequent_mask = torch.triu(
        subsequent_mask, diagonal=1
    )  # 대각선 기준으로  위에가 0인상태대각은 모두 1
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        position = (
            torch.arange(
                dec_inputs.size(1), dtype=dec_inputs.dtype, device=dec_inputs.device
            )
        ...
        for layer in self.layers:
            ...
            dec_outputs, self_attn_prob, dec_enc_attn_prob = layer.forward(
