...
class DecoderLayer(nn.Module):
    def __init__(self, config):
        ...
        self.self_layer_norm1 = nn.LayerNorm(
            self.config.d_hidn, eps=self.config.layer_norm_epsilon
        )
        self.dec_enc_attn = MultiHeadAttention(self.config)
        self.self_layer_norm2 = nn.LayerNorm(
            self.config.d_hidn, eps=self.config.layer_norm_epsilon
        )
        self.pos_ffn = PoswiseFeedForwardNet(self.config)
        self.self_layer_norm3 = nn.LayerNorm(
            self.config.d_hidn, eps=self.config.layer_norm_epsilon
        )
    ...
    def forward(self, dec_inputs, enc_outputs, self_attn_mask, dec_enc_attn_mask):
        self_att_outputs, self_attn_prob = self.self_attn.forward(
        ...
        dec_enc_att_outputs, dec_enc_attn_prob = self.dec_enc_attn.forward(
            self_att_outputs, enc_outputs, enc_outputs, dec_enc_attn_mask
        )
        dec_enc_att_outputs = self.self_layer_norm2(
            dec_enc_att_outputs + self_att_outputs
        )
