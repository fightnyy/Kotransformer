...
def build_data_loader(vocab, infile, args, shuffle=True):
    ...
    if 1 < args.n_gpu and shuffle:
        ...
        loader = torch.utils.data.DataLoader(
            dataset,
            batch_size=config.tconfig.batch_size,
            sampler=sampler,
...
def movie_collate_fn(inputs):
    ...
    enc_inputs = nn.utils.rnn.pad_sequence(
        enc_inputs, batch_first=True, padding_value=0
    )
    dec_inputs = nn.utils.rnn.pad_sequence(
    ...
    def __init__(self, vocab, infile):
        ...
        with open(infile, "r") as f:
            for i, line in enumerate(
                tqdm(f, desc=f"Loading {infile}", total=line_cnt, unit=" lines")
            ):
